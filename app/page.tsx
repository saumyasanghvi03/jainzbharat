      ...
      <section className="mx-auto max-w-7xl px-4 py-20 md:px-6 md:py-28">
        <div className="glass rounded-[2rem] p-8 shadow-glow md:p-14">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link className="rounded-full bg-white px-5 py-3 font-semibold text-black hover:bg-gold focus:outline-none focus:ring-2 focus:ring-gold" href={routes.declaration}>
              Read Declaration
            </Link>
            <Link className="rounded-full border border-white/15 px-5 py-3 font-semibold hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-gold" href={routes.signingWall}>
              View Signing Wall
            </Link>
          </div>
        </div>
      ...
      <Section eyebrow="Ecosystem" title="Every module is designed as a real product surface, not a placeholder.">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {platformModules.map((module) => (
            <Link key={module.slug} href={module.href} className="rounded-3xl border border-white/10 bg-white/[.035] p-6 transition hover:border-gold/60 hover:bg-white/[.06] focus:outline-none focus:ring-2 focus:ring-gold">
